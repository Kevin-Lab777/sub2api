...
const createAccountAndFinish = async (
  ...
  if (activeHoursEnabled.value) {
    const ahExtra: Record<string, unknown> = { ...(finalExtra || {}) }
    ahExtra.active_hours = [{ start: activeHoursStart.value, end: activeHoursEnd.value }]
    finalExtra = ahExtra
  }
  await doCreateAccount({
    name: form.name,
...
const handleAnthropicExchange = async (authCode: string) => {
  ...
  try {
    ...
    // Add active hours settings
    if (activeHoursEnabled.value) {
      extra.active_hours = [{ start: activeHoursStart.value, end: activeHoursEnd.value }]
    }
