...
// [LITE] 标准模式不检查余额：检查 user×platform quota、API Key/Group 限额、API Key rate limit 和 RPM。
...
func (s *BillingCacheService) CheckBillingEligibility(ctx context.Context, user *User, apiKey *APIKey, group *Group, subscription *UserSubscription, platform string) error {
	...
	// [LITE] 标准分组模式：检查 API Key 限额。优先 API Key 自身限额，再检查 Group 限额。
	if apiKey != nil {
		if err := s.checkAPIKeyLimitEligibility(ctx, apiKey, group); err != nil {
			return err
		}
	}
...
func (s *BillingCacheService) checkRPM(ctx context.Context, user *User, group *Group) error {
	if s == nil || user == nil || user.ID <= 0 || s.userRPMCache == nil {
		return nil
	}
	...
	if override == nil && groupID > 0 && s.userGroupRateRepo != nil {
		if loaded, err := s.userGroupRateRepo.GetRPMOverrideByUserAndGroup(ctx, user.ID, groupID); err == nil {
			override = loaded
		} else {
			logger.LegacyPrintf("service.billing_cache", "get user-group rpm override failed user=%d group=%d: %v", user.ID, groupID, err)
		}
	}

	if override != nil {
		if *override > 0 {
			count, err := s.userRPMCache.IncrementUserGroupRPM(ctx, user.ID, groupID)
			if err != nil {
				logger.LegacyPrintf("service.billing_cache", "increment user-group rpm failed user=%d group=%d: %v", user.ID, groupID, err)
				return nil
			}
			if count > *override {
	...
	if groupID > 0 && groupLimit > 0 {
		count, err := s.userRPMCache.IncrementUserGroupRPM(ctx, user.ID, groupID)
		if err != nil {
			logger.LegacyPrintf("service.billing_cache", "increment group rpm failed user=%d group=%d: %v", user.ID, groupID, err)
			return nil
		}
		if count > groupLimit {
			return ErrGroupRPMExceeded
...
func (s *BillingCacheService) checkUserRPM(ctx context.Context, user *User) error {
	if s == nil || user == nil || user.ID <= 0 || user.RPMLimit <= 0 || s.userRPMCache == nil {
		return nil
	}
	count, err := s.userRPMCache.IncrementUserRPM(ctx, user.ID)
	if err != nil {
		logger.LegacyPrintf("service.billing_cache", "increment user rpm failed user=%d: %v", user.ID, err)
		return nil
	}
