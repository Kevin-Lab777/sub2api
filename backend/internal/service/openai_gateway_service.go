...
import (
	...
	"github.com/Kevin-Lab777/sub2api/internal/pkg/apicompat"
	...
	if isSubscriptionBilling {
		if shouldBill && cost.TotalCost > 0 {
			if err := s.userSubRepo.IncrementUsage(ctx, subscription.ID, cost.TotalCost); err != nil {
				logger.LegacyPrintf("service.openai_gateway", "Increment subscription usage failed: %v", err)
			}
		...
		if hasAPIKeyLimits || hasGroupLimits {
			if shouldBill && cost.ActualCost > 0 && s.apiKeyUsageRepo != nil {
				if err := s.apiKeyUsageRepo.IncrementUsage(ctx, apiKey.ID, cost.ActualCost); err != nil {
					logger.LegacyPrintf("service.openai_gateway", "Increment API key usage failed: %v", err)
				}
			}
		} else {
