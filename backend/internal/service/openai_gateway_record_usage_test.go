...
func TestRecordCyberPolicyUsageLog_RecordsRealUpstreamTokenCost(t *testing.T) {
	...
	// 流式 cyber：上游 response.failed 报告了真实 token，须记录真实成本，
	...
	require.Equal(t, 0, userRepo.deductCalls, "Lite 标准模式记录成本但不扣余额")
}
...
func TestOpenAIGatewayServiceRecordUsage_UpdatesQuotaWhenUsageLogCreateReturnsError(t *testing.T) {
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		...
		APIKey:        &APIKey{ID: 10041, Quota: 100},
		User:          &User{ID: 20041},
...
func TestOpenAIGatewayServiceRecordUsage_UsageLogWriteErrorDoesNotSkipQuotaAccounting(t *testing.T) {
...
func TestOpenAIGatewayServiceRecordUsage_QuotaAccountingUsesDetachedContext(t *testing.T) {
