...
func TestOpenAISelectTechnicalChatCompletionsDirectRequiresAPIKeyAccount(t *testing.T) {
	t.Parallel()
	...
	selection, err := svc.SelectTechnicalChatCompletionsDirectAccountWithLoadAwareness(context.Background(), nil, "", "gpt-5.4", nil)
	require.NoError(t, err)
	require.Equal(t, int64(2), selection.Account.ID)
}
...
func TestOpenAISelectTechnicalChatCompletionsAllowsSubscriptionAdapter(t *testing.T) {
	t.Parallel()
	...
	selection, err := svc.SelectTechnicalChatCompletionsAccountWithLoadAwareness(context.Background(), nil, "", "gpt-5.4", nil)
	require.NoError(t, err)
	require.Equal(t, int64(1), selection.Account.ID)
}

func TestOpenAISelectTechnicalChatCompletionsRejectsUnsupportedAccountType(t *testing.T) {
	t.Parallel()

	svc := &OpenAIGatewayService{
		accountRepo: stubOpenAIAccountRepo{accounts: []Account{
			{ID: 1, Platform: PlatformOpenAI, Type: AccountTypeSetupToken, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0},
			{ID: 2, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 1},
		}},
		concurrencyService: NewConcurrencyService(&technicalOpenAIConcurrencyCache{
			loadMap: map[int64]*AccountLoadInfo{1: {AccountID: 1, LoadRate: 0}, 2: {AccountID: 2, LoadRate: 0}},
...
func TestOpenAISelectTechnicalImagesDirectRequiresAPIKeyAccount(t *testing.T) {
	t.Parallel()
	...
	selection, err := svc.SelectTechnicalImagesDirectAccountWithLoadAwareness(context.Background(), nil, "", "image-alias", nil)
	require.NoError(t, err)
	require.Equal(t, int64(2), selection.Account.ID)
}
...
func TestOpenAISelectTechnicalLiveRequiresEligibleOAuthAccount(t *testing.T) {
	t.Parallel()

	svc := &OpenAIGatewayService{
		accountRepo: stubOpenAIAccountRepo{accounts: []Account{
			{ID: 1, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0},
			{ID: 2, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 1, Credentials: map[string]any{"auth_mode": OpenAIAuthModePersonalAccessToken}},
			{ID: 3, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 2, Credentials: map[string]any{"model_mapping": map[string]any{"live-alias": "gpt-live-upstream"}}},
		}},
		concurrencyService: NewConcurrencyService(&technicalOpenAIConcurrencyCache{
			loadMap: map[int64]*AccountLoadInfo{1: {AccountID: 1, LoadRate: 0}, 2: {AccountID: 2, LoadRate: 0}, 3: {AccountID: 3, LoadRate: 0}},
		}),
	}
	selection, err := svc.SelectTechnicalLiveAccountWithLoadAwareness(context.Background(), nil, "live-alias", nil)
	require.NoError(t, err)
	require.Equal(t, int64(3), selection.Account.ID)
}
