...
func (s *PaymentService) doBalance(ctx context.Context, o *dbent.PaymentOrder) error {
	// [LITE:DELETED] Balance recharge no longer creates or redeems codes.
	// Payment backend compatibility is retained only to complete existing orders.
...
func (s *PaymentService) tryClaimAffiliateRebateAudit(ctx context.Context, client *dbent.Client, orderID int64, baseAmount float64) (bool, error) {
	...
	exists, err := client.PaymentAuditLog.Query().
		Where(
			...
			paymentauditlog.ActionIn("AFFILIATE_REBATE_APPLIED", "AFFILIATE_REBATE_SKIPPED"),
		).
		Exist(ctx)
	if err != nil {
		return false, err
