...
import (
	...
	"github.com/Kevin-Lab777/sub2api/internal/pkg/openai_compat"
	"github.com/Kevin-Lab777/sub2api/internal/pkg/timezone"
	"github.com/Kevin-Lab777/sub2api/internal/pkg/xai"
)
...
// ActiveHourRule limits account scheduling to a daily hour window.
type ActiveHourRule struct {
	Start int `json:"start"`
	End   int `json:"end"`
}

// GetActiveHours reads Lite scheduling windows from the account extra payload.
...
func (a *Account) isInActiveWindow(now time.Time) bool {
	...
	for _, rule := range rules {
		...
		} else if hour >= rule.Start || hour < rule.End {
			return true
		}
	}
	return false
