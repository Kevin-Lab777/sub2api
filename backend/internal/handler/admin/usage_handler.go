...
// ResetAPIKeyUsage handles resetting API key usage counters
// POST /api/v1/admin/api-keys/:id/reset-usage
func (h *UsageHandler) ResetAPIKeyUsage(c *gin.Context) {
	if h.apiKeyUsageService == nil {
		response.Error(c, http.StatusServiceUnavailable, "API key usage service unavailable")
		return
	}
	...
	if err := h.apiKeyUsageService.ResetUsage(c.Request.Context(), apiKeyID, req.Period); err != nil {
		response.ErrorFrom(c, err)
		return
	}

	response.Success(c, gin.H{"id": apiKeyID, "period": req.Period, "status": "reset"})
}
